...
$(function () {
    ...
    // 判断浮点数value的精度2位
    jQuery.validator.addMethod("FloatPrecision", function (value, element) {
    ...
    jQuery.validator.addMethod("compareDate", function (value, element, param) {
        if (value == "" || param == "") {
            var starttime = new Date(param.replace(/-/g, "/"));
    ...
    }, $.validator.format("时间不能小于等于{0}"));
    //时间不能小于特定时间(参数为元素id)
    jQuery.validator.addMethod("compareDateByEle", function (value, element, param) {
        param = $(param).val();
        if (value == "" || param == "") {
            var starttime = new Date(param.replace(/-/g, "/"));
            var endtime = new Date(value.replace(/-/g, "/"));
    ...
    }, $.validator.format("时间不能小于等于起始时间！"));
    //数值不能小于特定数值(参数为元素id)
    jQuery.validator.addMethod("compareFloatByEle", function (value, element, param) {
        param = $(param).val();
        if (value == "" || param == "") {
            return this.optional(element) || false;
        } else {
            var start = parseFloat(param);
