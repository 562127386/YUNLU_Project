...
{	
	...
	/// TNRD_StockInBLL
	/// </summary>	
	public class TNRD_StockInBLL
	{
	    private ITNRD_StockInRepository service=new TNRD_StockInRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="TNRD_StockInCache" ;
        ...
        /// <returns></returns>
        public IEnumerable<TNRD_StockInEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<TNRD_StockInEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<TNRD_StockInEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<TNRD_StockInEntity> GetPageList(Pagination pagination, string queryJson)
        {
			var expression = LinqExtensions.True<TNRD_StockInEntity>();
            var queryParam = queryJson.ToJObject();
             //查询条件
        ...
        public void SaveForm(string keyValue, TNRD_StockInEntity tNRD_StockInEntity)
        {
            try
            {
                service.SaveForm(keyValue, tNRD_StockInEntity);
                CacheFactory.Cache().WriteCache(cacheKey,tNRD_StockInEntity);
            }
            catch (Exception)
            {
                throw;
