...
{	
	...
	/// TNRD_StockOutDBLL
	/// </summary>	
	public class TNRD_StockOutDBLL
	{
	    private ITNRD_StockOutDRepository service=new TNRD_StockOutDRepository();

		/// <summary>
        /// 缓存key
        /// </summary>
        public string cacheKey ="TNRD_StockOutDCache" ;
        ...
        /// <returns></returns>
        public IEnumerable<TNRD_StockOutDEntity> GetPageListBySql(Pagination pagination, string queryJson)
        {
        ...
        /// </summary>
        /// <returns></returns>
        public IEnumerable<TNRD_StockOutDEntity> GetList( string queryJson)
        {
             var expression = LinqExtensions.True<TNRD_StockOutDEntity>();
            var queryParam = queryJson.ToJObject();
            //查询条件
        ...
        /// <param name="queryJson">查询参数</param>
        /// <returns></returns>
        public IEnumerable<TNRD_StockOutDEntity> GetPageList(Pagination pagination, string queryJson)
        {
			var expression = LinqExtensions.True<TNRD_StockOutDEntity>();
            var queryParam = queryJson.ToJObject();
             //查询条件
        ...
        public void SaveForm(string keyValue, TNRD_StockOutDEntity tNRD_StockOutDEntity)
        {
            try
            {
                service.SaveForm(keyValue, tNRD_StockOutDEntity);
                CacheFactory.Cache().WriteCache(cacheKey,tNRD_StockOutDEntity);
            }
            catch (Exception)
            {
                throw;
